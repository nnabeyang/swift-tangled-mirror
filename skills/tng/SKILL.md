...
- Perform `pipeline retry` only when the user authorizes the write. Inspect the
  original pipeline and verify the repository and optional workflow first.
  Report the new pipeline ID and AT URI; do not repeat the command when the
  result is ambiguous until absence of the first write has been established.
