...
struct PipelineCancelCommand: AsyncParsableCommand {
  static let configuration = CommandConfiguration(
    commandName: "cancel",
    abstract: "Cancel a running Tangled CI pipeline"
  )
  ...
  @Option(
    ...
    help: "Cancel only the named workflow; repeat for multiple workflows"
  )
  var workflow: [String] = []

  @Flag(help: "Output the cancellation result as JSON")
  var json = false
  ...
  func run() async throws {
    try await runCLICommand(jsonErrors: json) {
      try await PipelineCommandService(formatter: .live).cancel(
